...
class PenjualanSalesMitraModel extends Model
{
    ...
    public function getpenjualansalmit(){
        ...
        ->select('penjualan_salmit.jumlah, penjualan_salmit.tgl_jual, penjualan_salmit.harga, penjualan_salmit.alamat_trank, salesnya_mitra.nama_salmit, stok_barang_mitra.nama_kategori, customer_salmit.nama_cussalmit, customer_salmit.no_telp_customer_salmit')
        ->where('mitra.email',['email'=> $data])
        ->get()->getResultArray();
    }
    public function getpenjualansales_mitra(){
        $session = session();
        $data = $session->get('email');
        return $this->db->table('penjualan_salmit')
        ->join('stok_barang_mitra','stok_barang_mitra.id_stokbarmit=penjualan_salmit.id_stokbarmit')
